  ...
  export default {
    ...
    methods: {
      ...
      handleOk () {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            formData.type = 'UserAccount'
            formData.keyIds = this.checkedKeys
            formData.oneValue = this.accountId
      ...
      loadTree(id) {
        let that = this
        that.treeData = []
        that.accountUserTree = []
        let params = {};
        params.id='';
        getAction('/user/getUserWithChecked?UBType=UserAccount&UBValue='+id).then((res) => {
          if (res) {
            //机构全选后，再添加机构，选中数量增多
            this.allTreeKeys = [];
            for (let i = 0; i < res.length; i++) {
              ...
              that.accountUserTree.push(temp)
